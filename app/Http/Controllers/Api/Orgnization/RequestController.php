...
class RequestController extends Controller
{
    ...
    //     with(['user:id,name,email','task:id,name','event:id,name','orgnization:id,name'])->find($id);
    //     if (!$request) {
    ...
    public function attendRequest(Request $request, $id){
        $requestData = ModelsRequest::find($id);
        if (!$requestData) {
            return response()->json(['message' => 'Request not found'], 404);
        }
    ...
    public function lostRequest(Request $request, $id){
        $requestData = ModelsRequest::find($id);
        if (!$requestData) {
            return response()->json(['message' => 'Request not found'], 404);
        }
        $requestData->status = 'lost';
        $requestData->save();
        return response()->json(['message' => 'Request status updated successfully'], 200);
    }
}
